...
def fetch_price_and_history(symbol: str) -> tuple[float | None, float | None, str | None]:
    ...
    (split adjustments, currency changes, pre-IPO data quirks). For our use
    case — detecting meaningful drawdowns for current decision-making — we
    only care about recent peaks, so 2020+ is sufficient.
    ...
    try:
        ...
        ath = float(hist["Close"].loc[ath_idx])
...
def build_heartbeat_for_group(config: dict, group_name: str, snapshots: list[dict]) -> str | None:
    ...
    header = f"📰 <b>Stocks Daily check — {group_name}</b>"
...
def is_heartbeat_time(config: dict, state: dict, force: bool = False) -> bool:
    """
    Returns True if it's time to send the daily heartbeat.

    If force=True, always returns True (used for manual testing via
    workflow_dispatch input).

    Otherwise: match if current local hour is within 1 of
    config['heartbeat_hour_local'], AND we haven't already sent today's.
    """
    if force:
        return True
    ...
    tz = ZoneInfo(config["timezone"])
    now_local = datetime.now(tz)
    target_hour = config.get("heartbeat_hour_local", 22)

    today_str = now_local.strftime("%Y-%m-%d")
    if state.get("last_heartbeat_date") == today_str:
        return False

    if abs(now_local.hour - target_hour) <= 1:
        return True
    return False
...
def main() -> int:
    ...
    if is_heartbeat_time(config, state, force=force_hb):
        ...
        if not messages:
            # Edge case: no group has anything to report. Send one "all quiet" message.
            tz = ZoneInfo(config["timezone"])
            now_local = datetime.now(tz)
            timestamp = now_local.strftime("%Y-%m-%d %H:%M %Z")
            text = (
                f"📰 <b>Stocks Daily check</b>\n"
                f"<i>{timestamp}</i>\n\n"
            ...
            for text in messages:
                send_telegram(bot_token, chat_id, text, silent=True)
        tz = ZoneInfo(config["timezone"])
        state["last_heartbeat_date"] = datetime.now(tz).strftime("%Y-%m-%d")
    else:
